...
$semuadata2 = array();
$semuadata3 = array();
$terkirim = $conn->query("SELECT * FROM laporan,mahasiswa,kategori,status_laporan WHERE laporan.keluhan LIKE '%$keyword%' and status_laporan.status='terkirim' and status_laporan.id_status=laporan.id_status and laporan.nim=mahasiswa.nim and laporan.id_kategori=kategori.id_kategori");
$approve = $conn->query("SELECT * FROM laporan,mahasiswa,kategori,status_laporan WHERE laporan.keluhan LIKE '%$keyword%' and status_laporan.status='approve' and status_laporan.id_status=laporan.id_status and laporan.nim=mahasiswa.nim and laporan.id_kategori=kategori.id_kategori");
$unapprove = $conn->query("SELECT * FROM laporan,mahasiswa,kategori,status_laporan WHERE laporan.keluhan LIKE '%$keyword%' and status_laporan.status='unapprove' and status_laporan.id_status=laporan.id_status and laporan.nim=mahasiswa.nim and laporan.id_kategori=kategori.id_kategori");
...
<!-- mencari data -->
<?php if (empty($semuadata1 or $semuadata2 or $semuadata3)) : ?>
...
<?php foreach ($semuadata2 as $key => $value) : ?>
  <div>
    ...
    <div class="form">
      <textarea placeholder="feedback kosong" <?php if( $value['feedback']){ echo "readonly";} ?> name="feedback" id="feedback" cols="10" rows="1"><?php echo $value['feedback']; ?></textarea>
    </div>
    </form>
  </div>
<?php endforeach ?>
